...
## Summarize the temp and precipitation of the Greenland nest sites for the two sub-populations
## Create average temp, summed precipitation for a pre breeding period, post breeding period and full period
## Model how climate effects productivity in two sub-populations


## packages required, use pacman to speed up read in
library(pacman)
p_load(ggplot2, data.table, lubridate, tidyverse, MuMIn, nlme, lmtest, effects, performance, ggpubr)
...
setwd("Plots")
ggsave("Figure 5- Temporal trends in producitvity.png", 
...
fitITop$Trend <- ifelse(fitITop$Trend == 1, "Decreasing", "Increasing")
...
fitITop2$Trend <- ifelse(fitITop2$Trend == 1, "Decreasing", "Increasing")
...
I2 <- ggplot(mapping=aes(x= All_precip, y = Young, group = Trend, colour = Trend)) + 
      ...
      geom_point(data = Clim_Islay2, alpha = 0.5) +
      xlab("Total precipitation (April 1st – August 31st)/mm") + ylab("Number of juveniles per 1000, Islay") +
      labs(colour="Global trend") +
      theme_bw() +
      scale_colour_manual(values=c("#E1BE6A", "#40B0A6")) +
      theme(panel.grid.minor.y = element_blank(),
...
setwd("Plots")
ggsave("Figure 6- Islay Freezedays and precip days vs producitvity.png", 
...
fitWTop$Trend <- ifelse(fitWTop$Trend == 1, "Decreasing", "Increasing")
...
fitWTop2$Trend <- ifelse(fitWTop2$Trend == 1, "Decreasing", "Increasing")
...
W2<-ggplot(mapping=aes(x= pre_hatch_precip, y = Young, group = Trend, colour = Trend)) + 
    ...
    geom_point(data = Clim_Wexf2, alpha = 0.5) +
    xlab("Total Precipitation (April 1st – June 20th)/mm") + ylab("Number of juveniles per 1000, Wexford") +
    labs(colour="Global trend") +
    theme_bw() +
    scale_colour_manual(values=c("#E1BE6A", "#40B0A6")) +
    theme(panel.grid.minor.y = element_blank(),
...
## now arrange both of the plots onto a single plot
ggarrange(W1, W2, common.legend = T, nrow = 2)

setwd("Plots")
ggsave("Figure 7- Wexford Freezedays and Precip vs producitvity.png", 
       width = 25, height = 30, units = "cm")
